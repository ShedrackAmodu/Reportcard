    ...
    event.respondWith(
      caches.open(DYNAMIC_CACHE).then(async cache => {
        try {
          ...
          // If we have data cached in IndexedDB for this model, return it as JSON
          try {
            const model = getModelFromUrl(url.pathname);
            if (model) {
              const storedContext = await getStoredUserContext().catch(() => null);
              const schoolId = storedContext && storedContext.schoolId ? storedContext.schoolId : null;
              const cachedData = await getCachedData(model, schoolId).catch(() => null);
              if (cachedData && cachedData.length > 0) {
                return new Response(JSON.stringify({ results: cachedData }), {
                  status: 200,
      ...
      .then(response => {
        ...
        return fetch(request)
          .then(response => {
            ...
            // Don't cache chrome-extension and other non-http requests
  ...
  switch (type) {
    ...
    case 'GET_CACHED_DATA':
      getCachedData(data.model, data.schoolId)
        .then(cachedData => reply(cachedData))
        .catch(error => reply(null));
      break;

    case 'GET_CONFLICTS':
      getConflicts()
        .then(conflicts => reply(conflicts))
        .catch(error => reply([]));
      break;
    ...
    case 'CACHE_API_RESPONSE':
      if (data.model && data.data) {
        ...
        reply({ success: false, error: 'Invalid data format' });
      }
      break;
